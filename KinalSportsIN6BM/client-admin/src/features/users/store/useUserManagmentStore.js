    ...
    updateUserRole: async (userId, roleName) => {
        ...
        try {
            if (typeof updateUserRoleRequest !== "function") {
                throw new Error("La función de actualización de rol no está disponible")
            }

            const { data: updatedUser } = await updateUserRoleRequest(userId, roleName)
            ...
            set({
                error: err.response?.data?.message || err.message || "Error al actualizar rol",
                loading: false
            })
            return {
                success: false,
